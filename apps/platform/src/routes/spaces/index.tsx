import { useState, useMemo } from "react";
import { useSpaces } from "../../modules/spaces/hooks/use-spaces";
import { SpaceList } from "../../modules/spaces/components/space-list";
import { MapPlaceholder } from "../../modules/spaces/components/map-placeholder";

export function SpacesPage() {
  const [area, setArea] = useState("");
  const { spaces, isLoading, error } = useSpaces({ area: area || undefined });

  // Build unique area list from all spaces (fetch without filter for dropdown)
  const { spaces: allSpaces } = useSpaces();
  const areas = useMemo(
    () => Array.from(new Set(allSpaces.map((s) => s.area))).sort(),
    [allSpaces],
  );

  return (
    <div className="space-y-5">
      {/* Header */}
      <div>
        <h1 className="text-lg font-bold text-gray-900">Spaces</h1>
        <p className="text-xs text-gray-400 mt-0.5">
          Temukan tempat kerja bareng yang pas
        </p>
      </div>

      {/* Map placeholder */}
      <div className="h-48 rounded-xl overflow-hidden border border-gray-100 shadow-sm">
        <MapPlaceholder />
      </div>

      {/* Filter + count */}
      <div className="flex items-center justify-between gap-3">
        <select
          value={area}
          onChange={(e) => setArea(e.target.value)}
          className="text-xs border border-gray-200 rounded-lg px-3 py-2 text-gray-600 bg-white focus:border-indigo-500 focus:outline-none"
        >
          <option value="">Semua Area</option>
          {areas.map((a) => (
            <option key={a} value={a}>{a}</option>
          ))}
        </select>

        {!isLoading && !error && (
          <p className="text-xs text-gray-400">{spaces.length} space ditemukan</p>
        )}
      </div>

      {/* Space list */}
      <SpaceList spaces={spaces} isLoading={isLoading} error={error} />
    </div>
  );
}
