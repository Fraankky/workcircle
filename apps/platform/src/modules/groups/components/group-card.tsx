...
export function GroupCard({ group }: GroupCardProps) {
  return (
    <Link to="/groups/$id" params={{ id: group.id }}>
      <div className="bg-[#161B22] rounded border border-[#30363D] hover:border-[#484F58] transition-all duration-150 overflow-hidden cursor-pointer">
        {/* Color accent bar */}
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <span
              className={`flex items-center gap-1 text-xs font-medium ${
                group.is_open ? "text-[#3FB950]" : "text-[#6E7681]"
              }`}
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${
                  group.is_open ? "bg-[#3FB950]" : "bg-[#6E7681]"
                }`}
              />
          ...
          {/* Name + description */}
          <div>
            <h3 className="text-sm font-semibold text-[#C9D1D9] line-clamp-1">
              {group.name}
            </h3>
            <p className="text-xs text-[#6E7681] mt-0.5 line-clamp-2 leading-relaxed">
              {group.description}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-[#21262D]">
            ...
            <span className="text-xs text-[#6E7681] font-medium whitespace-nowrap">
